...
Alp Tuna Dağdanaş: I tried to helped my group to find the best threshold value for adding a copy paste indicator for each student's each question. Also, I tried to find new keywords to search in order to enhance the model's accuracy. Moreover, I tried different clustering algorithms that we learned in the lecture such as DBSCAN, hierarchial clustering and k-means algorithms and use the best clustering algorithm (best in a sense where it resulted the highest R2 test accuracy score compared to other clustering algorithms). Additionally, I used silhouette score to determine which number of clusters would be the best and visualized the results of the silhouette score of the number of clusters in the range 2 and 11. Finally, I helped my group to overcome NaN values and put mean value for each NaN (Not a Number) entry in its respective column.
